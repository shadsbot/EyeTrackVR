...
class Ransac:
    ...
    def blob_tracking_fallback(self):
        ...
        for cnt in contours:
            ...
            yt = y + int(h / 2)
            xrlb = (
                xt - self.lkg_projected_sphere["center"][0]
            ) / self.lkg_projected_sphere["axes"][0]
            eyeyb = (
                yt - self.lkg_projected_sphere["center"][1]
            ...
            xl = float(
                ((xt - self.xoff) * eye_position_scalar) / (self.xmax - self.xoff)
            )
            xr = float(
                ((xt - self.xoff) * eye_position_scalar) / (self.xmin - self.xoff)
            )
            yu = float(
                ((yt - self.yoff) * eye_position_scalar) / (self.ymin - self.yoff)
            )
            yd = float(
                ((yt - self.yoff) * eye_position_scalar) / (self.ymax - self.yoff)
            )
